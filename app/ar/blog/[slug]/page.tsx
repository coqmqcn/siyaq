...
export async function generateStaticParams() {
  return allPosts.filter((p) => p.lang === "ar").map((p) => ({ slug: p.slug }));
}

export async function generateMetadata(
  { params }: { params: { slug: string } }
): Promise<Metadata> {
  const post = allPosts.find((p) => p.slug === params.slug && p.lang === "ar");
  if (!post) return {};
  const url = absUrl(`/ar/blog/${post.slug}`);
...
export default function PostPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <article className="container prose prose-invert py-16" dir="rtl" lang="ar">
      <h1>{post.title}</h1>
